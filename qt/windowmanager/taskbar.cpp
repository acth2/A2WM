...
void TaskBar::updateMinimizedWindows() {
    if (!QX11Info::isPlatformX11())
        return;

    Display *display = QX11Info::display();
    Atom hiddenAtom = XInternAtom(display, "_NET_WM_STATE_HIDDEN", False);
    Atom stateAtom = XInternAtom(display, "_NET_WM_STATE", False);
    ...
    if (XGetWindowProperty(display, root, stateAtom, 0, (~0L), False,
        ...
        for (unsigned long i = 0; i < nItems; ++i) {
            ...
            Atom *windowState;
            unsigned long stateItems, stateBytesAfter;
            int stateActualFormat;
            Atom stateActualType;

            if (XGetWindowProperty(display, window, stateAtom, 0, (~0L), False,
                                   XA_ATOM, &stateActualType, &stateActualFormat,
                                   &stateItems, &stateBytesAfter,
                                   reinterpret_cast<unsigned char **>(&windowState)) == Success) {
                bool isMinimized = false;
                for (unsigned long j = 0; j < stateItems; ++j) {
                    if (windowState[j] == hiddenAtom) {
                        isMinimized = true;
                        break;
                    }
                }
                ...
                if (isMinimized && !minimizedWindows.contains(window)) {
...
void TaskBar::addMinimizedWindow(Window window) {
    QPushButton *windowButton = new QPushButton(this);
    windowButton->setText(QString("Window %1").arg((quint64)window));
    windowButton->setStyleSheet("border: 1px solid #000; background: #ccc;");
    windowButton->setFixedHeight(30);

    layout()->addWidget(windowButton);

    minimizedWindows.insert(window, windowButton);

    connect(windowButton, &QPushButton::clicked, this, [this, window]() {
        restoreMinimizedWindow(window);
    });
}

void TaskBar::restoreMinimizedWindow(Window window) {
    if (!QX11Info::isPlatformX11())
        return;
    ...
    event.xclient.data.l[1] = hiddenAtom;
    ...
    if (minimizedWindows.contains(window)) {
        QPushButton *button = minimizedWindows.take(window);
